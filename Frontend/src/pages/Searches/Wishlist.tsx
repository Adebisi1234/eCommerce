...
const Search = () => {
  ...
  const Allclocks = useTasks().clocks.filter((clock) =>
    location.includes("wishlist")
      ...
      : user?.orders.includes(clock.name)
  );
  ...
  return (
    <>
      <header className="h-[35px] mt-8 flex items-center justify-between">
        <div className="name flex items-center gap-[9px] ml-2">
          ...
          <h1 className="ml-[9px] font-extrabold text-xl text-[var(--color-dark)]">
            {location.includes("wishlist") ? "Wishlist" : "Orders"}
          </h1>
        </div>
      ...
      <div className="grid grid-cols-2 gap-x-2 w-full gap-y-5 mb-[58px]">
        {clocks.length > 0 ? (
          clocks.map((clock) => {
          ...
          <div className="h-full w-full flex justify-center items-center">
            No Items found, please{" "}
            {location.includes("wishlist")
              ? "add some products to your wishlist"
              : "buy some of our products"}
          </div>
        )}
      </div>
